...
public class SaveSmartAuto implements ITowerOfHanoi {

    private int[][] playingField;
    private int counter = 0;
    private int currentMaxAvailableRingSize = 0;
    private int lastRingSizeTransfer;

    public SaveSmartAuto(int numberOfRings, int numberOfColumns) {
        int ringSize = 1;
    ...
    public int getColumnWithMaxSizeAvailableRing() {
        ...
        for (int i = 0; i < this.playingField[0].length; i++) {
            for (int[] row : this.playingField) {
                if (row[i] != 0) {
                    if (isCanMakeMoveThisRing(row[i]) && row[i] != this.lastRingSizeTransfer) {
    ...
    public int getTargetColumnForTransfer() {
        ...
        for (int i = 0; i < this.playingField[0].length; i++) {
            for (int[] row : this.playingField) {
                if (row[i] != 0) {
                    if (row[i] > this.currentMaxAvailableRingSize) {
                        return i;
                    }
                    break;
                }
